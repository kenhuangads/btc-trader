...
function tradeCard(t, isOpen) {
  ...
  const exits = (t.exits || []).map(e => {
    const rn = {
      ...
      stagnation: "⏳停滯出場(無進展)", protect_stop: "🔒鎖利停損", profit_stop: "💰利潤棘輪出場",
    }[e.reason] || e.reason;
...
function renderSystem() {
  ...
  const pLabels = [
    ...
    ["trail_atr_mult", "移動停損（ATR）", v => v + "×（TP1 後啟動，輔助）"],
    ["ratchet_mfe_r", "鎖利棘輪（TP1 前）", v => `浮盈 ${v}R 後，停損上移到 −${P.ratchet_lock_r}R`],
    ["profit_ratchet_r", "利潤棘輪（TP1 後）", v => v ? `停損跟著浮盈爬，最多讓出 ${v}R 回吐` : "停用"],
  ...
  $("#philosophy").innerHTML = [
    ...
    ["浮盈不變虧、死單不戀戰", "進場後浮盈曾達 0.6R，停損自動上移到 −0.25R（鎖利棘輪）；進場 5 天毫無進展直接離場（停滯出場）——資金與注意力留給會動的行情。"],
    ["到手的利潤不還回去", "到第 1 目標後，餘倉的停損跟著浮盈往上爬，最多只讓出 0.6R 的回吐（利潤棘輪，逐根 K 更新）。復盤發現原本的 ATR 吊燈換算後永遠寬於 1R、實際從未生效，高浮盈的單常以保本收場（44 筆平均回吐 0.7R）——改用 R 計價的棘輪直接封住回吐上限，同時保留餘倉讓趨勢單繼續跑。"],
